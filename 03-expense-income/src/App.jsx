...
function App() {
  ...
  const [showErors,setShowError]=useState({
    statement:false,
    amount:false
  });
  ...
  const renderTotal=()=>{
    if(total>0){
      return <h1 className="total-text sucess">+{Math.abs(total)}</h1>;
    }
    else if(total <0){
  ...
  const handelUpdateInput=(e)=>{
    setInput({
      ...input,//destructuring
      [e.target.name]:e.target.value//replacing the target name with target value
    });
  };
     ...
     <div>
      { renderTotal()}
       ...
       <input
        ...
        onChange={handelUpdateInput}
        value={input.statement}
        name="statement"
        style={showErors.statement?{borderColor:"rgb(206,76,76)"}:null}
       />
       <input type="number"
        onChange={handelUpdateInput}
        value={input.amount}
        name="amount"
        style={showErors.amount?{borderColor:"rgb(206,76,76)"}:null}
          />
       <select onChange={handelUpdateInput}
